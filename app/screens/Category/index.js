...
import {StyleSheet,View, ScrollView,Button,Text, TouchableOpacity,FlatList,SafeAreaView, Pressable,RefreshControl, ActivityIndicator, Dimensions} from 'react-native';
...
function index({cart:{ cartItems }, navigation, route: {params}}) {
  const { t, i18n } = useTranslation();
   ...
   const [postPerload] = useState(10000);
    ...
    return (
      <ScrollView horizontal showsHorizontalScrollIndicator={true} style={{ marginBottom: scale(15) }}>
        {subcat.map((item) => (
          <TouchableOpacity
            key={item.id}
            onPress={() => navigation.navigate('Subcategory', { 'maincat': params.item, item })}  // Navigate on press
            style={{
              paddingTop: 10,
              paddingBottom:10,
              paddingRight:10,
